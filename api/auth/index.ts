/**
 * Unified auth handler: POST /api/auth?action=login|signup, GET /api/auth (me)
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // POST → login or signup (determined by action query param or presence of 'name' field)
    if (req.method === 'POST') {
      const action = req.query.action as string || (req.body?.name ? 'signup' : 'login')
